				...
				try{
					...
					for(int i=0;i<no;i++)
					{
						...
						else if((eccx>condx||eccminx>condx)&&(eccy<=condy&&eccminy<=condy))
						{
							...
							xf.format("%s%.3f","\nMu/(fck*b*d*d)=",myarray[i][4]*1000000/(concrete*850*850*850));
							...
							if(myarray[i][7]!=0)
							{
								...
								xf.format("%s%.3f","\nP/fck=",myarray[i][7]);
								p=concrete*myarray[i][7];
								
									
									xf.format("%s%.3f","\nRequired % of Steel=",p);
									xf.format("%s%.3f","\nRequired area of Steel=",p*0.01*850*850);
						...
						if((eccy>condy||eccminy>condy)&&(eccx<=condx&&eccminx<=condx)){
							...
							if(myarray[i][5]<muxl)
							{
								...
								xf.format("%s%.3f","\nRequired Area of Steel=",p*0.01*myarray[i][1]*myarray[i][2]);
